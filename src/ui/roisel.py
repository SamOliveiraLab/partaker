...
from PySide6.QtWidgets import (QApplication, QDialog, QGraphicsView, QGraphicsScene, 
    ...
    def __init__(self, image_data):
        ...
        if isinstance(image_data, np.ndarray):
            if image_data.ndim == 2:  # Grayscale
                ...
                qimg = QImage(image_data.data, width, height, width * channels, QImage.Format_RGB888)
            self.pixmap = QPixmap.fromImage(qimg)
        else:  # Assume it's a file path
            self.pixmap = QPixmap(image_data)
        
        self.image_shape = (image_data.shape[0], image_data.shape[1]) if isinstance(image_data, np.ndarray) else (
